...
from smartmeter.cli.args import parse_args
from smartmeter.p1.data import Telegram
from smartmeter.p1.read import P1Connection
...
def main():
    ...
    conn = P1Connection(
        serial_config=smartmeter.configuration.templates.KAIFA_MA304
    )
    ...
    while True:
        try:
            line = (
                conn.serial_connection.readline()
            ...
            msg = "Exception while reading from serial connection: {}".format(
                str(e)
            )
            logger.fatal(msg)
            sys.exit(1)
        ...
        if len(line) >= 1 and line[0] == "!":
            logger.debug("End of telegram found")
            # End of telegram found!

            # Check if we also got a header. If not, let wait for another round:
            if not p1_data.has_header():
                # discard the data we had so far because it is incomplete:
                telegram.clear()
                logger.debug(
                    "End of telegram reached but we don't have a "
                    "header, wait for another"
                )
                ...
                logger.debug(
                    f"Content of telegram:\n{pformat(p1_data.telegram, indent=2)}"
                )
        ...
        if 0 < arguments.telegrams <= telegram_counter:
            # Break from the loop if a telegram limit is set and the limit is
            # reached:
            break
